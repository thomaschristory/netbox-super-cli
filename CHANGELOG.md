...
- **`nsc/_version.py`** — `__version__` bumped to `"1.0.0"` to match. The CLI reads from this module (`from nsc._version import __version__` in `nsc/cli/app.py:12`); the build reads `[project].version` from `pyproject.toml`. Both sources are now in lockstep.
