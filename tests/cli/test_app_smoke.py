...
def test_offline_with_no_cache_or_bundled_exits_three(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, httpx2_mock: respx.Router
) -> None:
    monkeypatch.setenv("NSC_HOME", str(tmp_path))
...
def test_refresh_schema_flag_forces_refetch_through_full_cli(
    ...
    schema_route = httpx2_mock.get("https://nb.example/api/schema/?format=json").respond(
        200, json=schema_doc
    )
    httpx2_mock.get("https://nb.example/api/dcim/devices/").respond(200, json=devices_response)
